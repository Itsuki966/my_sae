...
def test_config():
    ...
    try:
        from config import (DEFAULT_CONFIG, LIGHTWEIGHT_CONFIG, TEST_CONFIG, 
                           MAC_CONFIG, SERVER_MEDIUM_CONFIG, SERVER_LARGE_CONFIG, 
                           get_auto_config)
        ...
        print(f"✅ 自動選択設定 - モデル: {auto_config.model.name}")
        print(f"✅ 自動選択設定 - デバイス: {auto_config.model.device}")
        print(f"✅ 自動選択設定 - サンプルサイズ: {auto_config.data.sample_size}")
...
def test_analyzer_init():
    ...
    try:
        ...
        print(f"✅ 最適化されたサンプルサイズ: {analyzer.config.data.sample_size}")
